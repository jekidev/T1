...
export interface McpServerDefinition { id: string; name: string; transport: "platform-connection" | "http" | "stdio" | "internal"; endpoint?: string; enabled: boolean; capabilities: string[]; requiresApprovalForWrites: boolean; }
export interface AiWorkspaceProfile {
  id: string;
  name: string;
  description: string;
  routing: { mode: LlmRoutingMode; staticModel: string; models: string[] };
  systemPrompt: string;
  rules: string[];
  skills: string[];
  memories: AiMemory[];
...
export const DEFAULT_AI_PROFILE: AiWorkspaceProfile = {
  ...
  routing: { mode: "rotate", staticModel: "nvidia/nemotron-nano-9b-v2:free", models: ["nvidia/nemotron-nano-9b-v2:free", "google/gemma-4-26b-a4b-it:free", "mistralai/mistral-nemo"] },
  systemPrompt: "You are the Game Co-Designer inside Urban Strategy Simulator. Play with the user, observe live state and telemetry, explain what is happening, and propose controlled improvements.",
  rules: [
  ...
  memories: [{ id: "project-purpose", title: "Project purpose", content: "The game is played with AI and developed with AI through an observe-propose-apply workflow.", enabled: true }],
  mcpServers: DEFAULT_MCP_SERVERS,
};

function cloneProfile(profile: AiWorkspaceProfile): AiWorkspaceProfile { return JSON.parse(JSON.stringify(profile)) as AiWorkspaceProfile; }
function mergeMcpDefaults(profile: AiWorkspaceProfile): AiWorkspaceProfile {
  ...
  return { ...profile, mcpServers: DEFAULT_MCP_SERVERS.map(server => ({ ...server, ...(existing.get(server.id) ?? {}) })) };
}

export function loadAiProfiles(): AiWorkspaceProfile[] {
  if (typeof window === "undefined") return [cloneProfile(DEFAULT_AI_PROFILE)];
  try { const stored = JSON.parse(localStorage.getItem(STORAGE_KEY) ?? "[]") as AiWorkspaceProfile[]; return stored.length > 0 ? stored.map(mergeMcpDefaults) : [cloneProfile(DEFAULT_AI_PROFILE)]; }
  catch { return [cloneProfile(DEFAULT_AI_PROFILE)]; }
}
export function saveAiProfiles(profiles: AiWorkspaceProfile[]) { localStorage.setItem(STORAGE_KEY, JSON.stringify(profiles)); }
export function loadActiveAiProfileId(): string { return localStorage.getItem(ACTIVE_KEY) ?? DEFAULT_AI_PROFILE.id; }
export function saveActiveAiProfileId(id: string) { localStorage.setItem(ACTIVE_KEY, id); }
export function createAiProfile(source: AiWorkspaceProfile = DEFAULT_AI_PROFILE): AiWorkspaceProfile { const suffix = Math.random().toString(36).slice(2, 7); return { ...cloneProfile(source), id: `${source.id}-${suffix}`, name: `${source.name} Copy` }; }
...
export function buildAiProfileContext(profile: AiWorkspaceProfile): string {
  ...
  return [
    ...
    `Available user assets:\n${workspace.assets.map(item => `- id=${item.id}; sourceId=${item.sourceId}; name=${item.name}; mime=${item.mimeType}; origin=${item.origin}`).join("\n") || "- none"}`,
    "To bind an existing asset in Build mode, set entities[].assetId or playerProfile.avatarAssetId to one of the exact IDs above. Unknown IDs are ignored. Do not invent asset URLs.",
