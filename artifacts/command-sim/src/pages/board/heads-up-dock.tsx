...
function boardSignature(board: Record<string, unknown>): string {
  const simulation =
    board.simulation && typeof board.simulation === "object"
      ? (board.simulation as Record<string, unknown>)
      : {};
  const moveLog = Array.isArray(board.moveHistory) ? board.moveHistory : [];
...
export function HeadsUpDock({ board, behavior }: HeadsUpDockProps) {
  ...
  useEffect(() => {
    void fetch("/api/llm/headsup/current")
      ...
      .then((body: { notification?: HeadsUpNotification | null }) => {
  ...
  useEffect(() => {
    if (mode === "light" || !behavior || behavior === lastBehavior.current)
      return;
  ...
  if (collapsed) {
    return (
      <Button
        className="fixed bottom-3 right-3 z-40 h-10 shadow-xl"
        onClick={() => setCollapsed(false)}
      >
        <BellRing className="mr-2 h-4 w-4" />
        Heads up · {LLM_MODE_META[mode].label}
      </Button>
    );
  }
    ...
    <div className="fixed bottom-3 right-3 z-40 w-[min(420px,calc(100vw-24px))] rounded-xl border border-border bg-background/95 shadow-2xl backdrop-blur">
      <div className="flex items-center justify-between border-b border-border px-3 py-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-xs font-semibold">LLM Heads Up</div>
            <div className="text-[9px] text-muted-foreground">
              {LLM_MODE_META[mode].short}
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-1 p-2">
        {(Object.keys(LLM_MODE_META) as LlmUserMode[]).map((value) => (
          <Button
            key={value}
            size="sm"
        ...
        {latest ? (
          <>
            <div className="mb-2 flex items-center justify-between gap-2">
              <div className="text-xs font-semibold">{latest.title}</div>
              <Badge variant="outline" className="text-[9px]">
                {latest.kind}
              </Badge>
            </div>
            <div className="max-h-36 overflow-y-auto whitespace-pre-wrap text-[11px] leading-relaxed">
              {latest.body}
            </div>
            <div className="mt-2 space-y-1 text-[9px] text-muted-foreground">
              {latest.sourceTitle && <div>Source: {latest.sourceTitle}</div>}
              {latest.model && <div>Model: {latest.model}</div>}
              {latest.scriptPath && (
                <div className="flex items-start gap-1">
                  <Code2 className="mt-0.5 h-3 w-3 shrink-0" />
                  <span className="break-all">
                    Review-only PR proposal: {latest.scriptPath}
                  </span>
                </div>
              )}
            </div>
          ...
          <div className="text-[11px] text-muted-foreground">
            Waiting for the first hourly wisdom item from
      ...
      <div className="flex items-center justify-between border-t border-border px-3 py-2">
        ...
        >
          <RefreshCw
            className={`mr-1 h-3 w-3 ${working ? "animate-spin" : ""}`}
          />
          Now
        </Button>
      </div>
