...
import { Bot, CheckCircle2, ChevronLeft, ChevronRight, Map, MousePointer2, PanelRight, Sparkles } from "lucide-react";
...
export function GuidedTutorial({ open, onClose, scenarioName, city = "din valgte by", generatedSummary }: GuidedTutorialProps) {
  const [step, setStep] = useState(0);
  const steps = useMemo(() => [
    {
      title: "ELI5: Hvad er spillebrættet?",
      icon: Map,
      body: `Tænk på ${scenarioName} som et digitalt brætspil oven på et rigtigt kort over ${city}. Google Maps viser den virkelige geografi. Spillets zoner, personer, assets og hændelser ligger som et interaktivt lag ovenpå.`,
      action: "Zoom og panorér kortet. Brug World-knappen til at ændre by, radius og startområde.",
    },
    {
      title: "Placér og vælg ting",
      icon: MousePointer2,
      body: "Venstre side er din palette. Træk eller tilføj entities til brættet. Klik på en entity eller zone for at se og ændre dens egenskaber.",
      action: "Tilføj mindst én location, unit eller objective og vælg den på brættet.",
    },
    {
    ...
    },
    {
      title: "Klar",
      icon: CheckCircle2,
      body: "Du behøver ikke forstå alle systemer på én gang. Start med kortet, vælg én mission, spørg Nyx om næste skridt, og lad spillet reagere på dine valg.",
  ...
  return (
    <Dialog open={open} onOpenChange={value => { if (!value) onClose(); }}>
      <DialogContent className="sm:max-w-[620px] select-text">
        <DialogHeader>
          <div className="flex items-center gap-3"><div className="rounded-full bg-primary/15 p-2"><Icon className="h-5 w-5 text-primary" /></div><DialogTitle>{current.title}</DialogTitle></div>
        ...
        <DialogFooter className="flex justify-between sm:justify-between">
          ...
          {step < steps.length - 1 ? <Button onClick={() => setStep(step + 1)}>Next<ChevronRight className="ml-1 h-4 w-4" /></Button> : <Button onClick={onClose}><CheckCircle2 className="mr-1 h-4 w-4" />Start game</Button>}
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
