...
import { applyTeamDynamicsEvent, useBoardStore, readAutosaveTimestamp, simulateTurn, type BoardState, type PlayerTurnAction, type TeamSide } from "@/lib/game";
...
import { ArrowLeft, Save, Download, Clock, Loader2, Code2, MapPin, BarChart3, CircleHelp, Map as MapIcon, Layers3, Network, PanelsTopLeft } from "lucide-react";
...
export default function BoardPage() {
  ...
  const resolveBlackmail = (action: BoardBlackmailAction) => {
    ...
    if (result.accepted && simulation) {
      ...
      resolvedBoard = { ...resolvedBoard, simulation: { ...simulation, teamDynamics: applyTeamDynamicsEvent(simulation, { side, source: "blackmail", ...effect }) } };
    }
    if (resolvedBoard !== board) loadBoard(resolvedBoard, scenarioId, scenarioName, scenarioDescription);
    toast({ title: result.accepted ? "In-game blackmail resolved" : "Blackmail command rejected", description: result.message, ...(result.accepted ? {} : { variant: "destructive" as const }) });
  };
  const handleSyndicateChange = (next: BoardState) => {
    const world = next.simulation?.syndicateWorld;
    if (!world) { loadBoard(next, scenarioId, scenarioName, scenarioDescription); return; }
    loadBoard({ ...next, simulation: { ...next.simulation!, syndicateWorld: { ...world, syndicates: world.syndicates.map(applyDanishSyndicatePreset) } } }, scenarioId, scenarioName, scenarioDescription);
  };
