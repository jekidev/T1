...
import { ArrowLeft, Save, Download, Clock, Loader2, Code2, MapPin, BarChart3, CircleHelp } from "lucide-react";
...
export default function BoardPage() {
  ...
  const handleSave = async () => {
    if (isTutorial || !scenarioId) { toast({ title: "Cannot save tutorial scenario directly.", description: "Export it to save locally." }); return; }
    try { await updateMutation.mutateAsync({ id: scenarioId, data: { board: board as any } }); toast({ title: "Scenario saved successfully." }); }
  ...
  const handleExport = () => {
    ...
    a.href = url; a.download = `operation-${scenarioName.replace(/\s+/g, "-").toLowerCase()}-${new Date().toISOString().slice(0,10)}.json`;
  ...
  return (
    <div className="flex flex-col h-screen w-full overflow-hidden bg-background select-text">
      ...
      <div className="flex-1 overflow-hidden"><ResizablePanelGroup direction="horizontal">
        ...
        <ResizablePanel defaultSize={60}><ResizablePanelGroup direction="vertical"><ResizablePanel defaultSize={70}><div className="relative h-full w-full overflow-hidden"><WorldMapUnderlay /><div className="absolute inset-0 z-10"><CommandCanvas mapTemplateId={board.mapTemplateId} /></div></div></ResizablePanel><ResizableHandle withHandle /><ResizablePanel defaultSize={30} minSize={15}><ScoringPanel /></ResizablePanel></ResizablePanelGroup></ResizablePanel><ResizableHandle withHandle />
