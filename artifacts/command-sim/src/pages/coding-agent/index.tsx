...
import {
  ...
  approvedNetworkDomainsFromLabels,
...
export default function CodingAgentPage() {
  ...
  const networkReady = networkMode !== "ultra" || ultraConfirmed;
  ...
  const createRun = async () => {
    ...
    try {
      ...
      const domains = approvedDomains.split(/\r?\n|,/).map(value => value.trim()).filter(Boolean);
      const labels = networkPolicyLabels({ mode: networkMode, approvedDomains: domains });
      ...
      toast({ title: run.status === "rejected" ? "Task rejected by policy" : "Isolated agent run created", description: `${run.branchName} · ${networkMode}` });
    } catch (error) {
      ...
      <div className="mx-auto max-w-[1500px] space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div><h1 className="text-xl font-semibold">Controlled Coding Agent</h1><p className="text-xs text-muted-foreground">OpenHands/Aider → isolated branch → network gate → patch → tests → independent review → PR</p></div>
          </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="space-y-2 rounded border p-3">
                  ...
                  <Select value={networkMode} onValueChange={value => { setNetworkMode(value as AgentNetworkMode); setUltraConfirmed(false); }}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="ask_first">Ask First · default</SelectItem>
                      <SelectItem value="offline">Offline · model transport only</SelectItem>
                      <SelectItem value="ultra">Ultra · all internet approved</SelectItem>
                    </SelectContent>
                  </Select>
                  {networkMode === "ask_first" && <div className="space-y-1"><Label>Domains already approved by you, one per line</Label><Textarea value={approvedDomains} onChange={event => setApprovedDomains(event.target.value)} className="min-h-16 font-mono text-xs" placeholder="docs.example.com" /><p className="text-[10px] text-muted-foreground">Every other domain is blocked by the infrastructure proxy. The agent must stop and ask before another domain can be added to a new run.</p></div>}
                  {networkMode === "offline" && <p className="text-[10px] text-muted-foreground">Agent tools cannot browse. Only the configured model API transport remains reachable.</p>}
                  {networkMode === "ultra" && <label className="flex items-start gap-2 rounded border border-amber-500/50 p-2 text-[10px]"><input type="checkbox" checked={ultraConfirmed} onChange={event => setUltraConfirmed(event.target.checked)} className="mt-0.5" /><span>I explicitly approve unrestricted internet access for this isolated run. Sources, licenses and dependency changes still require review.</span></label>}
                </div>

                <div className="rounded border p-2 text-[10px] text-muted-foreground"><ShieldAlert className="mr-1 inline h-3.5 w-3.5" />Protected paths, secrets, default-branch writes, untested patches and network-policy bypasses are rejected.</div>
...
function RunInspector({ run, busy, onExecute, onStop }: { run: AgentRun | null; busy: boolean; onExecute: (run: AgentRun) => Promise<void>; onStop: (run: AgentRun) => Promise<void> }) {
  ...
  const mode = networkModeFromLabels(run.task.labels);
  const domains = approvedNetworkDomainsFromLabels(run.task.labels);
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        <Section title="Policy"><div className="grid gap-2 text-xs md:grid-cols-5"><Value label="Accepted" value={String(run.policyDecision?.accepted ?? false)} /><Value label="Risk" value={run.policyDecision?.risk ?? "unknown"} /><Value label="Network" value={mode} /><Value label="Human review" value={String(run.policyDecision?.requiresHumanReview ?? true)} /><Value label="External reviewer" value={String(run.policyDecision?.requiresExternalReviewer ?? false)} /></div>{domains.length > 0 && <p className="mt-2 text-[10px] text-muted-foreground">Approved domains: {domains.join(", ")}</p>}{run.policyDecision?.reasons.map(reason => <p key={reason} className="mt-1 text-[10px] text-muted-foreground">{reason}</p>)}</Section>
